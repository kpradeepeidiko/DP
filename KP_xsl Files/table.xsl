...
<xsl:template name="generateTable">
	<xsl:variable name="query">
		SELECT ROLLNO,STDNAME FROM DB2INST1.STUDENT
	</xsl:variable>
...
xhttp.open("GET", "http://192.168.2.161:2079?id=123", true);
  xhttp.send();
}
</script>
</head>
			...
			<table border="1">
				...
				<xsl:for-each select="$data/sql/row">
					<tr>
						...
						<td><button onclick="loadDoc1('"+<xsl:value-of select="column[name='ROLLNO']/value"/>+"')">DELETE</button></td>
					</tr>
				</xsl:for-each>
				<tr>
